...
{
    $arr = [];
    $count = $ maxnumbers; while ($count > 0) {
    $arr[] = (rand(0, $maxnumbers));
    $count--;
} rsort($arr); foreach ($arr as $arrsort) {
    echo $arrsort;
    echo "
    <br>";
} } ?>
...
foreach ($geo as $oblast => $cityes) {
    ...
    if ("mo" == $oblast) {
        ...
        echo "
        <h4>Иркутская область</h4>";
    } elseif ("tomsk" == $oblast) {
        echo "
        <h4>Томская область</h4>";
    }
